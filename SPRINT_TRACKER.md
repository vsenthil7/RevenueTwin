...
| S69 | Guided first-run onboarding | On load, an empty case queue (no data imported yet) routes the user straight to the Import tab with a welcome banner (see your recoverable revenue in under 2 minutes), instead of an empty triage queue. Existing cases -> normal triage. | done (executed here) | 2 jsdom |
| S70 | Deployment hardening | docker-compose: env-substituted secrets (POSTGRES_*), fail-fast :? guards, OIDC env passthrough, app healthcheck - no more changeme. .env.example + .gitignore for .env. deploy.sh: fail-closed on missing/placeholder secrets, builds, then health-gates on /api/health. DEPLOYMENT.md + KNOWN_GAPS: HTTPS via TLS-terminating reverse proxy documented. compose validated via docker compose config. | done (executed here) | n/a (deploy artifacts; 601 backend still green) |
| S71 | ROI/payback calculator UI | web/app.js roiCalculatorHTML: interactive board-ready ROI panel on the CFO dashboard with editable inputs (annual platform cost, analyst hours saved, hourly cost) + Recalculate -> POST /api/roi -> live net value, return multiple, payback months, total benefit. See YOUR payback on YOUR numbers. | done (executed here) | 1 jsdom (+1 updated) |
| S72 | Explain-the-number provenance panel | web/api.js mapCase now exposes a per-finding breakdown (expected, actual, gross, decay, net, confidence); web/app.js provenanceBreakdownHTML renders an Explain-the-number table in the inspector showing the full deterministic derivation (expected-actual=gross, then legal/temporal decay -> defensible net). The anti-black-box differentiator. | done (executed here) | 2 jsdom |
| S73 | Scheduled re-scan + diff alerts | src/scheduler/rescan.ts diffScans: pure diff of a baseline scan vs current cases -> new/grown/shrunk/resolved deltas + new/resolved/net totals. app.rescanDiff compares a baseline snapshot to current persisted cases and records a durable rescan.completed audit event when something changed. Standing watchdog on top of the existing scheduler/alerts modules. | done (executed here) | 6 unit + 3 functional |
| S74 | Multi-currency import + consolidation | app.consolidatedRecoverable: collects every finding net-recoverable across all persisted cases and FX-converts them into one reporting-currency total via the consolidation module (FxRateBook + consolidate), with a per-currency breakdown. The importer keeps its clean single-currency-per-file invariant; multi-currency is handled at the portfolio/consolidation layer (the honest design - the per-import total stays correct). | done (executed here) | 1 functional |
...
| S83 | ContractIntentMismatch entity (spec gap closed) | src/intent/contract-intent-mismatch.ts: first-class scored mismatch between a Work IQ commercial intent and the contract term (uplift_not_in_contract / discount_not_in_contract / quantity_mismatch). Deterministic detect + score; presents BOTH contract and intent positions; resolveMismatch requires a human to pick a side (never auto-resolves); isOpen gate. The spec-named most original reasoning artifact. | done (executed here) | 12 unit @ 100/100/100/100 |
| S84 | SEED_MANIFEST.md + precision/recall (spec gap closed) | SEED_MANIFEST.md enumerates every planted defect (NW-QBR-UPLIFT Work-IQ-only + deterministic price/usage leaks) with ground-truth labels + detecting capability. src/metrics/detection.ts: score(planted,detected) -> precision/recall/F1 + assertNoRegression gate. Manifest-backed functional test proves the quantified Work IQ moat delta (recall 1 on, 0 off). | done (executed here) | 9 unit + 2 functional @ 100% |
| S85 | THREAT_MODEL.md STRIDE + adversarial (spec gap closed) | THREAT_MODEL.md: full STRIDE table (spoofing/tampering/repudiation/info-disclosure/DoS/EoP) with each mitigation mapped to a specific named test, incl prompt-injection-in-contract. New tests/negative/prompt-injection.test.ts proves an injected instruction is inert and cannot inflate the deterministic uplift. | done (executed here) | 2 adversarial |
| S86 | Stryker mutation gate >=90% (spec gap closed) | @stryker-mutator/core wired with stryker.conf.json scoped to the deterministic core (money, reconciliation, contract-intent-mismatch, detection), command runner over the targeted core tests, break threshold 90. Strengthened tests kill arithmetic/conditional/equality/sort/loop mutants (uneven-weight allocate, age-decay direction, window boundary, f1 harmonic mean, regression-gate boundary, half-away rounding). StringLiteral (error-message) mutator excluded as documented equivalent-mutant exemption. Score 92.77% (>=90). npm run mutation. | done (executed here) | mutation 92.77%; full gate 665 @ 100% |
| S87 | SPEC GAP: accessibility (WCAG 2.2 AA + axe-core CI) | Shared standard E.4 requires axe-core automated checks in CI on web + mobile. Not wired. | planned (spec gap) | TBD |
| S88 | SPEC GAP: per-agent specification docs + multi-agent race test | V03 sec 5 requires a spec template per agent (Mission/Inputs/Outputs/Deterministic boundary/Human gate/Failure mode/Audit events/Tests) and a tested two-agent conflicting-write race. Idempotency exists; the named race test + agent spec docs do not. | planned (spec gap) | TBD |
| S89 | SPEC GAP: screen-state matrix coverage | V03 sec 7 enumerates 12 states per core screen (loading/empty/permission-denied/partial-sync/source-unavailable/conflicting-evidence/low-confidence/approval-pending/etc). Audit which states are actually rendered+tested; fill gaps. | planned (spec gap) | TBD |
| S90 | SPEC GAP: M365 Copilot declarative agent surface | Track tool is M365 Copilot; V03 sec 10 wants a declarative agent + commands (show revenue risk for account X; prepare evidence pack for case Y) + Graph ingestion framing. Conceptually present via Work IQ but no declarative-agent manifest artifact. NEEDS USER: Azure/M365 tenant to wire live; can ship manifest + fallback offline. | planned (spec gap) | TBD |
| S83 | Golden Thread: Northwind Traders (CRITICAL) | Seed the named Northwind account with the March QBR 12%-uplift leak detectable ONLY via Work IQ intent; walk ingest->reconcile->corroborate->quantify->approve->write->dispute->partial-recovery->audit->dashboard delta as a CI-gated end-to-end test. Redefines submittable per spec section I. | planned | TBD |
| S84 | SEED_MANIFEST.md + precision/recall/F1 (CRITICAL) | Enumerate every planted defect with ground-truth label + detecting capability; compute precision/recall/F1 vs manifest with a regression gate. Spec sections A.5 + E.6; feeds Accuracy 20%. | planned | TBD |
| S85 | Blind-vs-sighted moat delta surfaced | Publish Work IQ disabled-recall X% vs enabled-recall Y% on dashboard + README; planted QBR case must vanish with Work IQ off. Spec sections B.1 + 10. | planned | TBD |
| S86 | THREAT_MODEL.md (STRIDE) standalone | Promote threat model to a standalone STRIDE file with mitigations mapped to specific tests. Spec section E.5. | planned | TBD |
| S87 | ContractIntentMismatch first-class case | Own entity + confidence model + human-resolution workflow + audit for contract<->intent conflicts. Spec section 8 (most original artifact). | planned | TBD |
| S88 | Explicit IQ-layer naming + provenance | Surface Work IQ / Foundry IQ / Fabric IQ by name in UI provenance + integration-health screen + README rubric map. Best-Use-of-IQ prize. | planned | TBD |
| S89 | Data lineage trace_id invariant | Every rendered UI value resolves via trace_id to source records + deterministic computation; global test: no UI value without a resolvable lineage handle. Spec section J. | planned | TBD |
| S90 | Mutation testing (Stryker >=90%) + axe-core WCAG | Wire Stryker mutation gate on deterministic core; axe-core WCAG 2.2 AA CI gate. Spec sections C.2 + E.4. | planned | TBD |
| S91 | Multi-agent idempotency/race test | Two agents proposing conflicting corrections to one invoice resolve deterministically, with a test for the race. Spec section 5. | planned | TBD |
| S92 | Demo video + submission pack (NEEDS USER) | Record the demo-beats video; finalize public README rubric map + demo link. REQUIRES USER: video recording, and any Azure/M365/Entra credentials for live (vs fallback) IQ. | planned (needs user) | TBD |
